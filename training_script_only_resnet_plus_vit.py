...
from utils import *
from eval_utils import custom_evaluate, custom_evaluate_only_resnet, custom_evaluate_only_resnet_plus
...
class LightningModel(L.LightningModule):
    def __init__(self, th_encoder, vit_encoder, decoder, word_map, encoder_lr=1e-4, decoder_lr=4e-4, encoded_image_size=7):
        super().__init__()

        self.save_hyperparameters(ignore=["th_encoder","vit_encoder","decoder","word_map"])
    ...
    def test_step(self, batch, batch_idx):
        self.test_references, self.test_hypotheses = custom_evaluate_only_resnet_plus(batch, self.th_encoder, self.vit_encoder, 
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.Adam([
                ...
                {'params': filter(lambda p: p.requires_grad, list(self.th_encoder.parameters())+list(self.vit_encoder.parameters())), 'lr': self.encoder_lr}
            ], lr=self.decoder_lr)
        return {
        "optimizer": optimizer,
        "lr_scheduler": {
...
if __name__=='__main__':
    ...
    lightning_model = LightningModel(thumb_enc, vit_enc, decoder, word_map, encoder_lr=encoder_lr, decoder_lr=decoder_lr, encoded_image_size=encoded_image_size)
    callbacks = [
    ...
    logger = CSVLogger(save_dir="/scratch/ss4yd/logs_resnet_plus/", name=f"my_model")
